...
int serieLin::readTTY(char* rx_buffer, int nBytesToRead) {
    ...
    int lg = 254;
    memset(buffer, '\0', lg);
    ioctl(uartStream, FIONREAD, &aLire);
    usleep(400); //0.0005s
...
string serieLin::sendAT(const char* ATcommand, const char* repAttendu, unsigned int timeout) {
    ...
    int nBytesWritten, nBytesRead = 0;
    int rep = 0;
    memset(bufW, '\0', 254);
	...
	usleep(strlen(bufW)*500); // sleep enough to transmit the X plus
    ...
    if (nBytesRead > 0) {
        ...
        fprintf(stderr, "Erreur AT reception\n");
    couleur("0");
